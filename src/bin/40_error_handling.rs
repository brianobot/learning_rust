...
fn main() {
    ...
    let fallback = weather_report_3.clone().unwrap_or(WeatherReport("Default Weather Report".to_string()));
...
fn get_weather(_location: LatLng) -> Result<WeatherReport, String> {
    Ok(WeatherReport(format!("Info at Coordination({}, {})", _location.0, _location.1)))
}